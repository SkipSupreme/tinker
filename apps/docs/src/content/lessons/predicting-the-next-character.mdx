...
summary: A language model is just a table, one row per "what came before" and one column per "what could come next." Learn the table by counting; sample from a row to generate text.
estimatedMinutes: 14
order: 1
prereqs:
...
<StepCheck
  id="predicting-the-next-character#chain-rule-joint-ad"
...
<StepCheck
  id="predicting-the-next-character#bigram-row-m-prob-a"
