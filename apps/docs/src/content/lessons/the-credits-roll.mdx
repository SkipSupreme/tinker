...
<StepChoice
  id="the-credits-roll#what-you-built"
  title="Name the artifact"
  single
  hint="A trained model is code that defines the computation plus weights that set the behavior."
>
When this course says you built a tiny transformer, what is the actual artifact?

<Choice>A prompt template wrapped around someone else's hosted model.</Choice>
<Choice correct>Source code that defines the computation, plus learned weights you can save and reload.</Choice>
<Choice>A static lookup table with every possible Shakespeare-like sentence prewritten.</Choice>
</StepChoice>
