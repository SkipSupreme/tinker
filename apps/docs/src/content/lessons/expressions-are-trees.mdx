...
Type `3 + 4 * 2 ** 2` into a Python prompt and into a JavaScript console. Both say `19`. Not `196`, not `49` — `19`, every time, on every machine.
...
A neural network is an arithmetic expression. A very wide one — billions of leaves and branch points instead of four — but the same kind of object: numbers at the bottom, operations stacked above.
