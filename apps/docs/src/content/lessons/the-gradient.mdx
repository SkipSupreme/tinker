...
One-line proof. Let $\mathbf{r}(t)$ trace a curve that stays on a level set, i.e., $f(\mathbf{r}(t)) = c$ for all $t$. Differentiate both sides with respect to $t$. The right side is a constant, so $d/dt = 0$. The left side, by a chain rule we'll formalize next module, is $\nabla f(\mathbf{r}(t)) \cdot \mathbf{r}'(t)$. So $\nabla f \cdot \mathbf{r}'(t) = 0$, meaning the gradient is perpendicular to the tangent direction of any curve sitting on a level set.
