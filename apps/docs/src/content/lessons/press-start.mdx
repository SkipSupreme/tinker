...
- The reference checkpoint for lesson 18.4 (not loaded until then). About 840 KB of `Float32Array` weights plus a 512-byte JSON header. We'll meet it when we need it.
...
After a few hundred iterations the loss will fall through ~3.0 (English character bigram statistics roughly captured), then through ~2.5 (word shapes), and bottom out somewhere around 1.6 to 1.8 if you let the full 2,000 run. That is the moment the validation curve diverges from the training curve and the model starts overfitting tiny-shakespeare, which is the right behavior at this scale.
